          ...
          children: [
            ...
            Column(
              ...
              children: certificationsItems.map((e) {
                int index =  certificationsItems.indexWhere((item) => item == e);
                return OnHover(
                  builder: (isHovered) {
                    return ListTile(
                          ...
                          child: Padding(
                            ...
                            child: ListTile(
                              title: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text(e.name, style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontSize: size.isMobile? 16: null),),
                                  Text(e.institution,style: Theme.of(context).textTheme.bodyMedium,)
                                ],
                              ),
                              subtitle: !size.isMobile? Text("Credentials: ${e.credentials}"): Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text(e.dateOfIssue,style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold),),
                                  Text(e.credentials,style: const TextStyle(fontSize: 12),),
                                ],
                              ),
                              trailing: size.isMobile? const SizedBox():  Text(e.dateOfIssue),
                            ),
                          ),
                        ),
                      ),
